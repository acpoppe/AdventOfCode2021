...
func findPathCountWithTwice(fromRoom: String, visited: [String], twice: Bool) -> Int {
    var count = 0
    var newVisited = visited
    newVisited.append(fromRoom)
    var hasGoneTwice = twice
    if (fromRoom == "start" && visited.contains("start")) {
        return 0
    }
    if (!hasGoneTwice && visited.contains(fromRoom.lowercased())) {
        hasGoneTwice = true
    }
    ...
    for room in caveValues[fromRoom]! {
        if (!hasGoneTwice || !visited.contains(room.lowercased())) {
            count += findPathCountWithTwice(fromRoom: room, visited: newVisited, twice: hasGoneTwice)
        }
    }
...
print(findPathCountWithTwice(fromRoom: "start", visited: [], twice: false))
