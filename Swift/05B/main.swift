...
func addLinesToMap(lines: [Line]) {
    for line in lines {
        if (line.startPoint.x == line.endPoint.x) {
            ...
            var lineInProgress = true
            var xPoint = Int(line.startPoint.x)
            var yPoint = Int(line.startPoint.y)
            let xEndPoint = Int(line.endPoint.x)
            let yEndPoint = Int(line.endPoint.y)
            while(lineInProgress) {
                map[yPoint][xPoint] += 1
                if (xPoint == xEndPoint || yPoint == yEndPoint) {
                    lineInProgress = false
                    break
                }
                if (xPoint < xEndPoint) {
                    xPoint += 1
                } else if (xPoint > xEndPoint) {
                    xPoint -= 1
                }
                if (yPoint < yEndPoint) {
                    yPoint += 1
                } else if (yPoint > yEndPoint) {
                    yPoint -= 1
                }
            }
        }
