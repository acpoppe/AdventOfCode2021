...
var inputSubstring = AOC.getRawInputFromBundleFile(fileName).split(separator: "\n")[0]
var root = parse(&inputSubstring)
while reduce(root) {}

for line in AOC.getRawInputFromBundleFile(fileName).split(separator: "\n").dropFirst() {
    var copy = line
    let newRule = parse(&copy)
    root = Pair(left: .pair(root), right: .pair(newRule))
    while reduce(root) {}
}
print(root.description)
print(root.magnitude.description)
